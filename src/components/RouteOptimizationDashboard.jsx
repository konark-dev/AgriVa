...
export default function RouteOptimizationDashboard() {
  ...
  return (
    <div className="space-y-6 text-slate-800 font-sans p-1 sm:p-2">
      {/* Header */}
      <div className="bg-gradient-to-r from-slate-900 via-slate-800 to-emerald-950 text-white p-4 sm:p-6 rounded-3xl shadow-xl border border-slate-800 flex flex-wrap items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          <div className="w-12 h-12 rounded-2xl bg-emerald-500/20 border border-emerald-500/40 flex items-center justify-center font-bold text-2xl shadow-inner text-emerald-400">
            🌾
          </div>
          <div>
            <div className="flex items-center gap-2">
              <h1 className="text-xl sm:text-2xl font-black tracking-tight">KisanRoute Optima</h1>
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className="flex items-center gap-1.5 px-3 py-2 bg-amber-500/10 hover:bg-amber-500/20 text-amber-300 border border-amber-500/40 rounded-xl text-xs font-bold transition"
          >
      ...
      {/* Main Grid: Left inputs (5 cols), Right results (7 cols) */}
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-5 space-y-5">
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
            ...
            <div className="p-4 space-y-3 text-xs">
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-slate-600 font-medium mb-1">Buyer / Mandi Name</label>
                  <input
                    type="text"
                    value={deal.buyerName}
                    onChange={(e) => setDeal({ ...deal, buyerName: e.target.value })}
                    className="w-full px-3 py-2 border rounded-xl focus:ring-2 focus:ring-emerald-500 outline-none text-slate-800 font-semibold bg-white"
                  />
                </div>
                <div>
                  <label className="block text-slate-600 font-medium mb-1">Commodity</label>
                  <input
                    type="text"
              ...
              <div>
                <label className="block text-slate-600 font-medium mb-1">Destination Location</label>
                <input
                  type="text"
                  value={deal.destination}
                  onChange={(e) => setDeal({ ...deal, destination: e.target.value })}
                  className="w-full px-3 py-2 border rounded-xl focus:ring-2 focus:ring-emerald-500 outline-none text-slate-800 bg-white"
                />
              </div>

              <div className="pt-2 border-t border-slate-100 flex items-center justify-between text-slate-600">
                <span>Total Requirement Demand:</span>
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
            <div className="bg-slate-50 border-b border-slate-200 px-4 py-3 flex items-center justify-between">
              ...
              <button
                ...
                className="text-xs font-semibold text-emerald-700 bg-emerald-50 hover:bg-emerald-100 border border-emerald-300 px-2.5 py-1 rounded-lg transition flex items-center gap-1"
              >
                <Plus className="w-3 h-3" /> Add FPO
              </button>
            </div>

            <div className="p-4 space-y-3 max-h-[340px] overflow-y-auto custom-scrollbar">
              {fpos.map((fpo, index) => (
                <div
                  key={fpo.id}
                  className="bg-slate-50/80 p-3 rounded-xl border border-slate-200 relative text-xs hover:border-emerald-400 transition"
                >
                  <div className="flex items-center justify-between mb-2">
                    ...
                    {fpos.length > 1 && (
                      <button
                        ...
                        className="text-red-500 hover:text-red-700 font-semibold text-xs p-1 rounded hover:bg-red-50"
                        title="Remove FPO"
                      >
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    <div>
                      <label className="text-[10px] text-slate-500 font-medium">Farmer / FPO</label>
                      <input
                        type="text"
                        ...
                        className="w-full px-2.5 py-1 border rounded-lg text-slate-800 bg-white"
                      />
                    </div>
                    <div>
                      <label className="text-[10px] text-slate-500 font-medium">Quantity (kg)</label>
                      <input
                        type="number"
                        step="500"
                        ...
                        className="w-full px-2.5 py-1 border rounded-lg text-slate-800 font-bold bg-white"
                      />
                    </div>
                  </div>
                </div>
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
            <div className="bg-slate-50 border-b border-slate-200 px-4 py-3 flex items-center justify-between">
              <div className="flex items-center gap-2">
                <h2 className="text-xs font-bold text-slate-800 uppercase tracking-wider">🚛 Vehicle Fleet</h2>
              ...
              <button
                ...
                className="text-xs font-semibold text-blue-700 bg-blue-50 hover:bg-blue-100 border border-blue-300 px-2.5 py-1 rounded-lg transition flex items-center gap-1"
              >
            ...
            <div className="p-4 space-y-3 max-h-[280px] overflow-y-auto custom-scrollbar">
              {vehicles.map((veh, index) => (
                <div
                  key={veh.id}
                  className="bg-slate-50/80 p-3 rounded-xl border border-slate-200 relative text-xs hover:border-blue-300 transition"
                >
                  <div className="flex items-center justify-between mb-2">
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    <div>
                      <label className="text-[10px] text-slate-500 font-medium">Reg Number</label>
                      <input
                        type="text"
                    ...
                    <div>
                      <label className="text-[10px] text-slate-500 font-medium">Payload Capacity (kg)</label>
                      <input
                        type="number"
                        step="500"
          ...
          <button
            ...
            className="w-full py-4 bg-emerald-600 hover:bg-emerald-700 active:scale-[0.99] text-white font-extrabold rounded-2xl shadow-lg shadow-emerald-700/30 text-base flex items-center justify-center gap-2 transition disabled:opacity-60 cursor-pointer"
          >
            {isOptimizing ? (
              <>
                <div className="w-5 h-5 border-3 border-white border-t-transparent rounded-full animate-spin"></div>
                <span>Solving Multi-Vehicle Route...</span>
              </>
            ) : (
              <>
        ...
        <div className="lg:col-span-7 space-y-6">
          {dispatchStatus && (
            <div className="bg-emerald-50 border-2 border-emerald-300 rounded-2xl p-4 text-emerald-900 shadow-sm flex items-center justify-between">
              <div className="flex items-center gap-3">
                <CheckCircle2 className="w-6 h-6 text-emerald-600 shrink-0" />
                <div>
                  <h3 className="font-bold text-sm">Fleet Dispatched Successfully!</h3>
          ...
          {result && result.success ? (
            <div className="space-y-4 animate-in fade-in duration-300">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 xl:grid-cols-6 gap-3">
                <div className="bg-white p-3 rounded-2xl border border-slate-200 shadow-sm">
                  <div className="text-[10px] font-bold text-slate-500 uppercase">TOTAL FPOs</div>
                  <div className="text-2xl font-black text-slate-900 mt-0.5">{result.totalFposServed}</div>
                  <div className="text-[10px] text-slate-400">nodes pooled</div>
                </div>

                <div className="bg-white p-3 rounded-2xl border border-slate-200 shadow-sm">
                  <div className="text-[10px] font-bold text-slate-500 uppercase">TRUCKS USED</div>
                  <div className="text-2xl font-black text-blue-700 mt-0.5">{result.totalVehiclesUsed}</div>
                  <div className="text-[10px] text-slate-400">of {vehicles.length} in fleet</div>
                </div>
                ...
                <div className="bg-white p-3 rounded-2xl border border-slate-200 shadow-sm">
                  ...
                  <div className="text-[10px] text-slate-400">{(result.totalLoadKg / 1000).toFixed(1)} MT Net</div>
                </div>
                ...
                <div className="bg-white p-3 rounded-2xl border border-slate-200 shadow-sm">
                  ...
                  <div className="text-[10px] text-slate-400">highway corridor</div>
                </div>

                <div className="bg-white p-3 rounded-2xl border border-slate-200 shadow-sm">
                  <div className="text-[10px] font-bold text-slate-500 uppercase">TRAVEL TIME</div>
                  <div className="text-xl font-black text-slate-900 mt-0.5">
                    {Math.floor(result.totalDurationMinutes / 60)}h {result.totalDurationMinutes % 60}m
                  </div>
                  <div className="text-[10px] text-slate-400">incl. mandi load</div>
                </div>

                <div className="bg-emerald-50 p-3 rounded-2xl border border-emerald-200 shadow-sm">
                  <div className="text-[10px] font-bold text-emerald-800 uppercase">LOGISTICS COST</div>
              ...
              <div className="bg-white p-3 rounded-2xl border border-slate-200 flex flex-wrap items-center justify-between gap-3 shadow-sm">
                <div className="flex items-center gap-2 text-xs text-slate-600">
                  <span className="font-semibold text-slate-800">Computation:</span> {result.computationTimeMs}ms
                  <span className="text-slate-300">|</span>
                  <span className="font-semibold text-slate-800">Engine:</span> {result.routingEngine}
                </div>
                <button
                  onClick={handleAcceptRoute}
                  disabled={dispatchStatus === "DISPATCHED"}
                  className="px-4 py-2 bg-emerald-600 hover:bg-emerald-700 disabled:bg-emerald-400 text-white rounded-xl text-xs font-bold transition shadow-sm"
                >
                    ...
                    >
                      <div className="flex items-start justify-between pb-2 border-b border-slate-100">
                        <div>
                          <div className="flex items-center gap-2">
                            ...
                            ></span>
                            <h4 className="font-bold text-sm text-slate-900">{route.vehicleName}</h4>
                          </div>
                      ...
                      <div className="space-y-1.5 pt-1">
                        ...
                        {route.stops.map((stop, sIdx) => (
                          <div key={stop.id || sIdx} className="flex items-center justify-between text-xs p-1.5 rounded-lg bg-slate-50 border border-slate-100">
            ...
            <div className="bg-white p-8 rounded-3xl border border-slate-200 text-center space-y-3">
              ...
              <h3 className="font-bold text-slate-800 text-base">Ready for Route Optimization</h3>
